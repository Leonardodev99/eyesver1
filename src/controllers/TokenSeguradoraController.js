...
class TokenSeguradoraController {
  async store(req, res) {
    ...
    const seguradora = await Seguradora.findOne({ where: { email } });
  ...
  if(!(await seguradora.passwordIsValid(password))) {
    return res.status(401).json({
...
// Criando token para o usuario
  const { id } = seguradora;
  const token = jwt.sign({ id, email }, process.env.TOKEN_SECRET_SEGURADORA, {
...
export default new TokenSeguradoraController();
